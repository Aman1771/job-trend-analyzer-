# 📊 Job Trend Analyzer

### 🔍 Explore Real-Time Job Market Insights Using Adzuna API

The **Job Trend Analyzer** project fetches real-time job listings from the **Adzuna Jobs API** for trending tech roles like **Data Scientist**, **Machine Learning Engineer**, and **Cloud Engineer**.  
It then processes, cleans, and visualizes the data to reveal **salary trends**, **demand patterns**, and **in-demand technical skills**.

---

## 🧠 Overview

The job market in tech evolves rapidly — new tools and roles appear every few months.  
This project helps you **analyze live job data** and gain insights such as:
- Which roles are most in demand
- Which skills are mentioned most frequently
- How salaries compare across roles and locations
- Which regions offer the highest-paying positions

---

## 🚀 Features

✅ Fetches **live job listings** using Adzuna API  
✅ Collects job titles, company, location, salary range, and job description  
✅ Cleans and processes data using **Pandas**  
✅ Computes **average salary** per job  
✅ Extracts **in-demand skills** from descriptions  
✅ Saves results to a CSV for Power BI or Python-based visualization  
✅ Ready for integration with **Power BI dashboards** or **Matplotlib/Seaborn charts**

---

## 📦 Tech Stack

| Category | Tools / Libraries |
|-----------|-------------------|
| Data Source | [Adzuna Jobs API](https://developer.adzuna.com/) |
| Language | Python 3 |
| Data Handling | Pandas |
| API Requests | Requests |
| Visualization | Matplotlib, Seaborn, WordCloud |
| Optional | Power BI |

---

## 🧰 Installation

### 1️⃣ Clone the repository
```bash
git clone https://github.com/Aman1771/job-trend-analyzer.git
cd job-trend-analyzer
